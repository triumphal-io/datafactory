...
const SheetView = forwardRef(({ workbookId, sheetId, onSavingChange, onLastSavedChange, onNavigationChange, onSelectionChange, selectedModel = DEFAULT_AI_MODEL }, ref) => {
    ...
    const [selectInfoPanel, setSelectInfoPanel] = useState(null); // { row, col, dropdownRect }
    ...
    const handleDropdownToggle = useCallback((isOpen, info) => {
        if (isOpen && info) {
            setSelectInfoPanel({ row: info.row, col: info.col, dropdownRect: info.dropdownRect, openAbove: info.openAbove });
        } else {
            setSelectInfoPanel(null);
        }
    }, []);
    ...
    return (
        <>
            ...
            {/* Information Panel for Select/Multi-select cells */}
            {selectInfoPanel && (
                <div
                    className="cell-info-panel"
                    onMouseDown={(e) => {
                    ...
                    style={{
                        position: 'fixed',
                        top: selectInfoPanel.openAbove
                            ? undefined
                            : selectInfoPanel.dropdownRect.bottom + 3,
                        bottom: selectInfoPanel.openAbove
                            ? window.innerHeight - selectInfoPanel.dropdownRect.top + 3
                            : undefined,
                        left: selectInfoPanel.dropdownRect.left,
                        width: selectInfoPanel.dropdownRect.width,
                        maxWidth: '400px',
                        maxHeight: '250px',
                        overflowY: 'auto',
                        zIndex: 999,
